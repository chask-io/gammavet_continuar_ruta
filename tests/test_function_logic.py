...
def test_continuar_ruta_claims_explicit_route_stop(monkeypatch):
    ...
    assert tenant_client.calls[0]["path"] == TENANT_START_NEXT_PATH
    assert tenant_client.calls[0]["json"]["action"] == "continuar"
    ...
    assert "Ruta R-1 continuada" in result
    whatsapp_call = next(c for c in fake_orchestrator.calls if c.get("event_type") == "response_to_whatsapp_message")
    assert whatsapp_call["prompt"].startswith("Siguiente parada de retiro")


def test_continuar_ruta_same_stop_noop_does_not_send_whatsapp(monkeypatch):
    tenant_client = FakeTenantClient(
        [{"claimed": False, "route_stop": _route_stop(), "total_stops": 3}]
    )
    fake_orchestrator = FakeOrchestrator()
    monkeypatch.setattr(
        "backend.conductor_common.ConductorContext.tenant_client",
    ...
    result = FunctionBackend(_event({"route_stop_id": ROUTE_STOP_ID})).process_request()

    assert "No-op" in result
    assert any(
        c.get("event_type") == "dispatch_event"
        and c["extra_params"]["event_type"] == "conductor_same_stop_noop"
        for c in fake_orchestrator.calls
    )
    assert not any(c.get("event_type") == "response_to_whatsapp_message" for c in fake_orchestrator.calls)
